...
void  staticMpiCalculateMandelbrot(double minX, double maxX, double minY, double maxY,int xPointsStart, int xPointsEnd, int *results){
    ...
    for (i=xPointsStart;i<xPointsEnd;i++){
        ...
        pointsRecording = pointsRecording+ numYPoints;
    }
}
...
void recordTimePoints(unsigned long long start_utime, unsigned long long end_utime){
    ...
    if (false){
        ...
        while (rank < numprocs)
        {
            if (myrank == rank)
            {
                fprintf(logFp,"###thread id=%d, total points=%d, Gettimeofday time = %llu.%03llu milisecond;  %llu.%06llu sec \n",myrank,pointsRecording, (end_utime - start_utime)/1000, (end_utime - start_utime)%1000, (end_utime - start_utime)/1000000, (end_utime - start_utime)%1000000  );
                fflush(stderr);
  ...
  sprintf(mylogbuffer,"###thread id=%d, total points=%d, Gettimeofday time = %llu.%03llu milisecond;  %llu.%06llu sec \n",myrank,pointsRecording, (end_utime - start_utime)/1000, (end_utime - start_utime)%1000, (end_utime - start_utime)/1000000, (end_utime - start_utime)%1000000  );
